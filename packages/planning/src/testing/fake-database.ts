...
/**
 * An in-memory stand-in for the Prisma/Database surface the planning code uses.
 *
 * No local Postgres exists in this repository's development loop, so the plan
 * service is exercised against this fake and cast to the real `Database` type
 * at the boundary. It reproduces the two properties the code depends on:
 *
 ...
 *  - `updateMany` returns a `count`, which is what the optimistic version claim
 *    keys on.
 *
 * Query support is deliberately just what the loader, persister and day-state
 * reader use: equality, `OR`, `in`, range operators, `not`, composite keys and
 * a single level of `include`.
 */
...
export class FakeStore {
  ...
  insert(model: string, row: FakeRow): FakeRow {
    ...
    // Prisma auto-generates ids (cuid) for rows that omit them; the audited
    // aggregate ids must be >= 8 characters, so the fake default must be too.
...
const matchesWhere = (row: FakeRow, where: WhereClause): boolean => {
  for (const [field, condition] of Object.entries(where)) {
    ...
    if (isPlainObject(condition)) {
      // Composite keys (e.g. `userId_date`) and nested filters match fields on
      // the row itself.
...
export class FakeTable {
  ...
  findUnique(options: { where: WhereClause }): FakeRow | null {
    return this.store.all(this.model).find((row) => matchesWhere(row, options.where)) ?? null;
  }
